...
describeIf('RedisConversationRepository Integration Tests', () => {
    ...
    it('should create a new conversation and retrieve its metadata by ID', async () => {
        ...
        expect(createdConversation.timestamp).toBe(timestamp);
    ...
    it('should update conversation metadata using the general update method', async () => {
        ...
        const initialData: ConversationData = { title: 'Before Update', projectId: initialProjectId, timestamp: initialTimestamp };
        const conversation = await conversationRepository.create(initialData);

        const updatedProjectId = 'proj-upd-final';
        const updatedTimestamp = (Date.now() + 1000).toString(); // Ensure different timestamp
        ...
        expect(updatedConversation.timestamp).toBe(updatedTimestamp);
    ...
    it('should set a new project ID and get the project ID', async () => {
        ...
        const initialData: ConversationData = { title: 'Project Linking Test', projectId: projectA, timestamp: Date.now().toString() };
        const conversation = await conversationRepository.create(initialData);
    ...
    it('should add messages and retrieve them', async () => {
        ...
        const conversationData: ConversationData = { title: 'Message Test Convo', projectId, timestamp: Date.now().toString() };
    ...
    it('should retrieve recent messages correctly', async () => {
        ...
        const conversationData: ConversationData = { title: 'Recent Message Test', projectId, timestamp: Date.now().toString() };
        const conversation = await conversationRepository.create(conversationData);
    ...
    it('should handle invalid JSON in messages gracefully', async () => {
        ...
        const conversationData: ConversationData = { title: 'Invalid Message Test', projectId, timestamp: Date.now().toString() };
        const conversation = await conversationRepository.create(conversationData);
    ...
    it('should add, remove, and get post IDs', async () => {
        const projectId = 'proj-post-link';
        const conversationData: ConversationData = { title: 'Post Linking Test', projectId, timestamp: Date.now().toString() };
        const conversation = await conversationRepository.create(conversationData);
        const postId1 = 'post-abc';
        const postId2 = 'post-def';
        ...
        expect(postIds).toEqual([postId2]); // Only post-def should remain
